...
$(document).ready(function()
{
     ...
     function giocone() 
    {
        ...
        $(document).keydown(function(e)
        {
            ...
            var stage=setInterval(creazione,600);
        ...
        function controlloFineGioco()
        {
            if(contMiss+punteggio == blocchettiTotaliStage)
            {
                ...
                var nuovoPunteggio=punteggioParziale+punteggio;

                localStorage.setItem('punteggio', nuovoPunteggio);
                clearInterval(stage);
                clearInterval(tick);
                $(".blocchetto").remove();
               //se non arrivo dal menu freestyle proseguo con la storia
               if( localStorage.getItem('arrivatoDaFreestyle') == 'false')    
               $(location).attr('href', 'vittoria.html');
                else
